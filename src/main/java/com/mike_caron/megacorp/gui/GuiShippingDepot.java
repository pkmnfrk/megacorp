    ...
    implements GuiButton.ClickedListener, GuiToggleButton.ChangedListener, GuiList.Producer
{
    ...
    public void clicked(GuiButton.ClickedEvent event)
    {
        ...
        else if(event.id == ContainerShippingDepot.GUI_REROLL_QUEST)
        {
            ...
            CtoSMessage packet = CtoSMessage.forGuiButton(container.getDimension(), container.getPos(), event.id, desiredItem);
            MessageHelper.send(packet);
        }
        else
        {
    ...
    {
        CtoSMessage packet = CtoSMessage.forGuiToggle(container.getDimension(), container.getPos(), event.id, event.newState);
        MessageHelper.send(packet);
    }
